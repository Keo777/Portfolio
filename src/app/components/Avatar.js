...
const Avatar = forwardRef(({ src, translateX, scale, opacity, zIndex, bottom, top, right, xlRight, xxlRight, left, width, height, index, total }, ref) => {
  ...
  return (
    <Image
      ...
      height={height}
      width={width}
      className={`opacity-${opacity} w-[clamp(500px,48vw,800px)] absolute bottom-${bottom} top-${top} right-${right} left-${left} xl:right-[${xlRight}] 2xl:right-[${xxlRight}] m-auto z-[${zIndex}] `}
